...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $project['total_projects']           = $this->userProjectCount($user->id, null);
        $project['total_pending_projects']   = $this->userProjectCount($user->id, 'pending');
        $project['total_accepted_projects']  = $this->userProjectCount($user->id, 'accepted');
        $project['total_rejected_projects']  = $this->userProjectCount($user->id, 'rejected');
        $project['total_reported_projects']  = $this->userProjectCount($user->id, 'reported');
        $project['total_completed_projects'] = $this->userProjectCount($user->id, 'completed');

        $gig['total_approved_gigs'] = Gig::where('user_id', $user->id)->approved()->count();
        $gig['total_pending_gigs']  = Gig::where('user_id', $user->id)->pending()->count();
        ...
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalWithdrawals', 'totalTransaction', 'countries', 'project', 'gig'));
    }


    protected function userProjectCount($userId, $scope = null)
    {
        if ($scope) {
        ...
        return $projects->where(function ($query) use ($userId) {
